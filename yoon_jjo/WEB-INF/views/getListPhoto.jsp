...
<body id="top">
	...
	<div id="main" style="padding-top: 3%">
		...
		<section id="two">
		<div id="photo">
				<a href="photo.do" class="btn btn-primary wirte-new" id="add-btn">추가</a>
				...
				<tbody>
						...
						<tr>
							<td>제목</td>
							<td><a href='getDetailPhoto.do?p_no=${photo.p_no}&curPage=${pager.curPage}&m_id=${sessionScope.member.m_id }'>${photo.p_title}</a></td>
						</tr>
						...
						<tr>
							<td></td>
							<td><img id="photoimg" src="resources/boardPhoto/${photo.bp_fullpath}"></td>
						</tr>
						<br>
						</tbody>
			</table>
			         ...
			         <ul class="pagination pageNo" >
			            <c:choose>
			               <c:when test="${pager.curPage!=1 }">
			                  <li class="no">
			                  <a  aria-label="Previous" href="getListPhoto.do?curPage=${pager.curPage-1 }">Previous</a></li>
			               </c:when>
			            </c:choose>
			            <c:forEach var="pageNo" begin="${pager.startPage }"
			               end="${pager.endPage }" step="1">
			               <li class=" page-item no"><a href="getListPhoto.do?curPage=${pageNo }">${pageNo }</a>
			               </li>
			            </c:forEach>
			            <c:choose>
			               <c:when test="${pager.curPage!=pager.lastPage }">
			                  <li class="no"><a
			                     href="getListPhoto.do?curPage=${pager.curPage+1 }">next</a></li>
			               </c:when>
			            </c:choose>
			         </ul>
			      </nav>
